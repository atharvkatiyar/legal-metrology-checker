...
class ExtractedField(BaseModel):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0)
    method: Literal["regex", "llm", "cv_calibration", "none"] = Field(...)
    notes: Optional[str] = Field(default=None) # Added to support Rule 9 coin calibration notes
...
class FontCheckInitResponse(BaseModel):
	model_config = ConfigDict(from_attributes=True)
	record_id: str
	image_url: str
	image_width: int
...
class FontCheckCalibrateResponse(BaseModel):
	model_config = ConfigDict(from_attributes=True)
	record_id: str
	available: bool
	message: Optional[str] = None
...
class FontCheckHistoryItem(BaseModel):
	model_config = ConfigDict(from_attributes=True)
	record_id: str
	created_at: str
	is_compliant: Optional[bool] = None
...
class FontCheckHistoryResponse(BaseModel):
	model_config = ConfigDict(from_attributes=True)
	total: int
	items: List[FontCheckHistoryItem] = Field(default_factory=list)