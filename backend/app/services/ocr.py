...
def _upscale_and_denoise(image):
    ...
    if shorter_side < LOW_RES_THRESHOLD:
        ...
        image = cv2.fastNlMeansDenoisingColored(image, None, h=7, hColor=7,
...
def _load_image(image_input: bytes | str) -> np.ndarray:
    
    if isinstance(image_input, bytes):
        stream = np.frombuffer(image_input, dtype=np.uint8)
        return cv2.imdecode(stream, cv2.IMREAD_COLOR)

    if isinstance(image_input, str):
        if os.path.exists(image_input):
            stream = np.fromfile(image_input, dtype=np.uint8)
            return cv2.imdecode(stream, cv2.IMREAD_COLOR)

        # Not a real file path — treat as base64
        data = image_input
        if "," in data:
            data = data.split(",")[1]
        decoded = base64.b64decode(data)
        stream = np.frombuffer(decoded, dtype=np.uint8)
        return cv2.imdecode(stream, cv2.IMREAD_COLOR)
